...
Two small JSON files, both included in this project (`api/` folder), both reusing your
**existing** `includes/db.php` — neither contains or duplicates your DB credentials:

1. **`api/news.php`** → powers the Home tab's "Latest News" list
2. **`api/home.php`** → powers the Home tab's hero banner (title/subtitle/buttons, pulled from
   your `banners` table where `page_key='home'`) and the "What Our Students Say" testimonials
   carousel (from your `testimonials` table)

Upload both to `/bethesdabiblicalinstitute/api/` on your server (create the `api` folder if it
doesn't exist). Test each by visiting them directly in a browser — you should see JSON. If a
banner/testimonials row doesn't exist yet or the request fails, the app falls back to sensible
default copy — it won't crash or show a blank screen.

## What's in the app

- Bottom navigation: **Home · Courses · Admissions · Profile**
- Splash screen + adaptive launcher icon (gold cross on navy)
- Home: hero banner **pulled live from your database** (title/subtitle/buttons, with safe
  fallback text), "Why Study With Us", live **Latest News** list, live **Student Testimonials**
  carousel
- Courses: search box + level filter chips (All/Certificate/Diploma/Bachelor's/Master's/Doctoral)
  over native cards for each program; tapping opens the full page on your site
